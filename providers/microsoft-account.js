...
exports.rpsSkypeTokenUri = "https://edge.skype.com/";
...
async function getLiveToken(options) {
    try {
        return requestLiveToken(options);
    } catch (_err) {
        const err = _err;
        switch (err.name) {
            case httpErrors.RequestError.name:
...
async function getLiveToken(options) {
    const uri = url_1.default.resolve(exports.liveLoginUri, path_1.default.posix.join("RST.srf"));
    const body = '<Envelope xmlns="http://schemas.xmlsoap.org/soap/envelope/" xmlns:wsse="http://schemas.xmlsoap.org/ws/2003/06/secext" xmlns:wsp="http://schemas.xmlsoap.org/ws/2002/12/policy" xmlns:wsa="http://schemas.xmlsoap.org/ws/2004/03/addressing" xmlns:wst="http://schemas.xmlsoap.org/ws/2004/04/trust" xmlns:ps="http://schemas.microsoft.com/Passport/SoapServices/PPCRL"> <Header> <wsse:Security> <wsse:UsernameToken Id="user"> <wsse:Username>' + options.username + '</wsse:Username> <wsse:Password>' + options.password + '</wsse:Password> </wsse:UsernameToken> </wsse:Security> </Header> <Body> <ps:RequestMultipleSecurityTokens Id="RSTS"> <wst:RequestSecurityToken Id="RST0"> <wst:RequestType>http://schemas.xmlsoap.org/ws/2004/04/security/trust/Issue</wst:RequestType> <wsp:AppliesTo> <wsa:EndpointReference> <wsa:Address>wl.skype.com</wsa:Address> </wsa:EndpointReference> </wsp:AppliesTo> <wsse:PolicyReference URI="MBI_SSL"></wsse:PolicyReference> </wst:RequestSecurityToken> </ps:RequestMultipleSecurityTokens> </Body> </Envelope>'
    const headers = { 'Content-Type': 'application/xml' };
    const postOptions = { uri, body, headers };
    const response = await options.httpIo.post(postOptions);
    if (response) {
        const responseParser = response.body.match(/\>t=(.*?);p=/);
        if (responseParser.length >= 2) {
            return responseParser[1];
        }
    }
}
...
async function getTokenSkype(options) {
    ...
    const formData = {
        partner: 999,
        scopes: "client",
        access_token: options.liveToken
    };
    const postOptions = {
        uri,
        form: formData,
    ...
    try {
        return httpIo.post(postOptions);
    } catch (err) {
        throw httpErrors.RequestError.create(err, postOptions);
    }
}
